...
```{r}
heart_tree_prune<-prune(heart_tree, optimalcp)
...
heart_tree$cptable
```
The heart disease classification tree was built using the rpart package, with a 70-30% train-test split and parameters minsplit = 10, minbucket = 3, and 10-fold cross-validation. The optimal complexity parameter (CP) was selected using the 1-SE rule, resulting in CP = 0.02158273, leading to a pruned tree with 5 splits. After pruning, the test misclassification error was 24.18% (0.2418), based on a confusion matrix showing 43 true negatives, 15 false positives, 7 false negatives, and 26 true positives. The final tree was then retrained on the entire dataset, and the Out-of-Bag (OOB) error was calculated as 38.85% (0.3885) from cross-validation at the optimal CP.
...
heart_xgb<-caret::train(AHD ~ ., method="xgbTree",
                        data = heart_train,
...
```
The XGBoost model's performance was evaluated using Mean Squared Error (MSE) and Root Mean Squared Error (RMSE), yielding an MSE of 0.2405 and an RMSE of 0.4905.
